#!/usr/bin/env python3
"""Validation Backtest: MS+OS SHORT-ONLY — Longs disabled.
...
Strategy:
  ...
  direction_filter = "short" — NO long entries (MS_ONL longs and OS_GAP_DN removed)

Validation run — compare against original (both directions):
  437 trades, PF 1.411, +$30,402, p=0.004, Sharpe 2.17

Usage:
    python3 scripts/os_ms_short_only.py
"""
import sys, os
...
OUTPUT_DIR = "output/os_ms_short_only_no_pvah"


def make_config():
    """MS Config B + OS Best — the deployment candidate."""
    cfg = StrategyConfig()
    cfg.direction_filter = "short"  # SHORT-ONLY — no long entries
...
print("=" * 80)
print("  VALIDATION BACKTEST: MS+OS SHORT-ONLY")
print("  Market Structure (ON + pVAH short) + Overnight Sweep (gap-up fade)")
print("  direction_filter = 'short' — NO long entries")
print("=" * 80)
print(f"""
